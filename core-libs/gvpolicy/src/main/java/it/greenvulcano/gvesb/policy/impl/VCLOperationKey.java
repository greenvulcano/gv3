...
public class VCLOperationKey implements ResourceKey
{
    ...
    {
        return "VCL#" + this.system + "#" + this.channel + "#" + this.operation;
    }
    ...
    public boolean match(ResourceKey resource)
    {
        if (resource instanceof VCLOperationKey) {
            if (!"".equals(this.system)) {
                if (!this.system.equals(((VCLOperationKey) resource).system)) {
                    return false;
                }
            }
            if (!"".equals(this.channel)) {
                if (!this.channel.equals(((VCLOperationKey) resource).channel)) {
                    return false;
                }
            }
            if (!"".equals(this.operation)) {
                if (!this.operation.equals(((VCLOperationKey) resource).operation)) {
                    return false;
                }
            }
            return true;
