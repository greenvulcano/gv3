...
{
	private static final String BASE_DIR = "target" + File.separator + "test-classes" + File.separator;
    ...
    //private static final String KEYSTORE_NAME            = "target/test-classes/GVEsb.jks";
    ...
    protected void setUp() throws Exception
    {
        super.setUp();
        try {
            FileUtils.moveFile(new File(BASE_DIR + KEYSTORE_NAME), new File(BASE_DIR
                    + KEYSTORE_NAME + ".orig"));
        }
        catch (Exception exc) {
    ...
    {
        new File(BASE_DIR + KEYSTORE_NAME).delete();
        new File("target/keystores/" + KEYSTORE_NAME).delete();
        try {
            FileUtils.moveFile(new File(BASE_DIR + KEYSTORE_NAME + ".orig"), new File(
                    BASE_DIR + KEYSTORE_NAME));
        }
        catch (Exception exc) {
            // TODO: handle exception
        }
    ...
    public void testXPathFunctions()
    {
        try {
            ...
            assertEquals("Bad current()/document() function", "AAAA__2", XMLConfig.get(n,
                    "gvf:document('${{gv.app.home}}/test-classes/testFunc2.xml')/root/elem[@id=current()/text()]"));
            /*try {
                Thread.sleep(190000);
            }
            catch (Exception exc) {
