...
if "%MAVEN_OPTS%"=="" SET MAVEN_OPTS="-Xms512m -Xmx1024m -XX:MaxPermSize=256m -Dmaven.artifact.threads=4"

%MAVEN_JAVA_EXE% %JVM_CONFIG_MAVEN_PROPS% %MAVEN_OPTS% %MAVEN_DEBUG_OPTS% -classpath %CLASSWORLDS_JAR% "-Dclassworlds.conf=%M2_HOME%\bin\m2.conf" "-Dmaven.home=%M2_HOME%" "-Dmaven.multiModuleProjectDirectory=%MAVEN_PROJECTBASEDIR%" %CLASSWORLDS_LAUNCHER% -s %M2_HOME%\conf\user-settings.xml %MAVEN_CMD_LINE_ARGS%
