 ...
 */
public class DBOMongoUpsert extends AbstractMongoDBO
{

	private static final Logger logger = GVLogger.getLogger(DBOMongoUpsert.class);
	...
	public void init(Node config) throws DBOException
	{
		...
		try {
			...
			NodeList stmts = XMLConfig.getNodeList(config, "MDBUpsert");
			String id = null;
			Node stmt;
			for (int i = 0; i < stmts.getLength(); i++) {
				stmt = stmts.item(i);
				id = XMLConfig.get(stmt, "@id");
				MDBUpsert mdbUpsert = new MDBUpsert();
				mdbUpsert.init(stmt);

				if (id == null) {
					id = Integer.toString(i);
				}
				statements.put(id, mdbUpsert);
			}
	...
	InterruptedException {
		try {
			prepare();
			logger.debug("Begin execution of DB data upsert through " + dboclass);
			...
			if (input != null) {
				...
				if( docJsonUpdate.containsKey("RowSet") ) {
					...
					for(org.bson.Document itemData : dataList) {
						...
						if(stmt != null) {
							...
							for(Document itemRow : rowList) {
								...
								writes.add(new UpdateManyModel<org.bson.Document>(docQuery, docUpdate, options));
							}

							bulkWriteResult = collection.bulkWrite(writes);
							
							int nUpserts = ( bulkWriteResult.getUpserts() != null) ? bulkWriteResult.getUpserts().size() : 0;
							rowInsOk += bulkWriteResult.getInsertedCount() + nUpserts;
							rowUpdOk += ( (nUpserts == 0) ? bulkWriteResult.getMatchedCount() : 0);							
					...
					// CASE 2: JSON Document defined as input without <RowSet> only one statement element.
					...
					if(stmt != null) {
						...
						bulkWriteResult = collection.bulkWrite(writes);
						System.out.println("BULK_WRITE_RESULT: "+bulkWriteResult);
				...
				if (stmt != null) {
					...
					writes.add(new UpdateManyModel<org.bson.Document>(docJsonQuery, docJsonUpdate, options));
			...
			logger.debug("End execution of DB data updsert through " + dboclass);
		}
	...
	public void execute(OutputStream data, MongoClient mongoClient,
			...
			InterruptedException {
		prepare();
		throw new DBOException("Unsupported method - DBOMongoUpsert::execute(OutputStream, MongoClient, HashMap)");
	}


	public class MDBUpsert implements IDBOOperation {
		private String id;
		...
		public MDBUpsert(String id, String dbName, String collection,
