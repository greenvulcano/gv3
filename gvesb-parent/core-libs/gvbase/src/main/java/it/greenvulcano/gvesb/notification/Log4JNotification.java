...
{
    private static Logger       logger         = GVLogger.getLogger(Log4JNotification.class);
     ...
     * Log4j fields
     */
    private Logger              notifyLogger   = null;
    ...
    public void init(Node notifyNode) throws XMLConfigException, GVNotificationException
    {
        ...
        if (msgLogger != null) {
            notifyLogger = GVLogger.getLogger(msgLogger);
        }
        else {
        ...
        try {
            ...
            try {
                ...
                if (traceException && (envInput instanceof Throwable)) {
                    notifyLogger.log(msgLevel, messageFormatter, (Throwable) envInput);
                }
                else {
