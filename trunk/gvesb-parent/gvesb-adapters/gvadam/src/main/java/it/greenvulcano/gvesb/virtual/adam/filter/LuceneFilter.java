...
public class LuceneFilter extends BaseFilter {
	...
	private static Logger logger = GVLogger.getLogger(LuceneFilter.class);
	...
	public void init(Node node) throws AdamAdapterException {
		...
		try {
			...
			filter = XMLConfig.get(node, "filter");
		} catch (XMLConfigException exc) {
			throw new AdamAdapterException("Error in LuceneFilter initialization", exc);
		}
	...
	public void filter(XMLUtils parser, Element root, ApplicationSession session, GVBuffer gvBuffer)
		...
		try {
			...
			String expandedFilter = PropertiesHandler.expand(filter, params, gvBuffer);

			SearchOperation searchOp = new SearchOperation(session, false, true);
			try {
				ElementCollection res = searchOp.simpleSearch(expandedFilter, sortBy);
				toXML(parser, root, session, res, returnType, linksTo, children, getClass().getSimpleName(), id, logger);
			} finally {
				searchOp.close();
			}
		} catch (Exception exc) {
			throw new AdamAdapterException("Error in LuceneFilter search operation", exc);
		}
