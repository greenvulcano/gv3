 ...
 */
public class LinksToFilter extends BaseFilter {

	private static Logger logger = GVLogger.getLogger(LinksToFilter.class);
	...
	public void init(Node node) throws AdamAdapterException {
		...
		try {
			...
			throw new AdamAdapterException("Error in LinksToFilter initialization", exc);
		}
	...
	public void filter(XMLUtils parser, Element root, ApplicationSession session, GVBuffer gvBuffer)
		...
		try {
			...
			try {
				ElementCollection res = searchOp.linksTo(expandeduuid);
				toXML(parser, root, session, res, returnType, null, null, getClass().getSimpleName(), id, logger);
			} finally {
				searchOp.close();
			}
		} catch (Exception exc) {
			throw new AdamAdapterException("Error in LinksToFilter filter operation", exc);
		}
