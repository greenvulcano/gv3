...
var address = baseURL + "gvaxis2/services/ProcessManagement";
...
ProcessManagementService.getProcessDefinition =
function getProcessDefinition(/* String */ _name, /* String */ _nmspURL, /* String */ _prefix)
{
    var isAsync, request, response, resultValue;

    this._options = new Array();
    isAsync = (this.getProcessDefinition.callback != null && typeof(this.getProcessDefinition.callback) == 'function');
    request =
    '<p:getProcessDefinition xmlns:p="http://www.apache.org/ode/pmapi">' +
    ((_name != null && _nmspURL != null && _prefix != null) ? '<pid xmlns:' + this._encodeXML(_prefix) + '="' + this._encodeXML(_nmspURL) + '">' +
                                                              this._encodeXML(_prefix) + ':' + this._encodeXML(_name) + '</pid>' : '') +
    '</p:getProcessDefinition>';
    ...
    if (isAsync) {
        try {
            this._call(
                    ...
                    new Array(this.getProcessDefinition.callback, this.getProcessDefinition.onError)
                    );
        } catch (e) {
            var error;
            if (WebServiceError.prototype.isPrototypeOf(e)) {
            ...
            this.getProcessDefinition.onError(error);
        }
    } else {
        try {
            response = this._call("getProcessDefinition", request);
            resultValue = /* tProcessInfo */ response.documentElement;
            return resultValue;
        } catch (e) {
            if (typeof(e) == "string") throw(e);
...
ProcessManagementService.getProcessDefinition.callback = null;

ProcessManagementService.undeploy =
function undeploy(/* String */ _name, /* String */ _nmspURL, /* String */ _prefix)
{
    var isAsync, request, response, resultValue;
