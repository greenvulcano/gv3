...
$.extend($.fn.bindTo,
    {
        ...
        traverse: function(key,data,template,repeaters,parent){
            if(typeof data == 'string' || typeof data == 'number' || typeof data == 'boolean'){
                ...
                return template.replace(new RegExp("\{"+key+"\}","g"),data);
            }else if(typeof data == 'object'){
                if(typeof data.length == 'undefined'){
                    if(repeaters['<!--'+parent+'-->'].action)
                        template = $.fn.bindTo[repeaters['<!--'+parent+'-->'].action](template,data,repeaters['<!--'+parent+'-->'].index)||template; // TODO
                    ...
                    for(var item in data){
                        ...
                        repeaters['<!--'+key+'-->'].index += 1;
                    }
                    repeaters['<!--'+key+'-->'].index = 0;
                    return listTemplate;
                }
            }
