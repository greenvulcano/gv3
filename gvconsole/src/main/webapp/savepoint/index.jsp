...
<%@ include file="../head.jspf" %>
    ...
    <style type="text/css">
       .sv-list {}
       .sv-list-H {background-color:lightgrey;}
        /*th { border: 1px solid #777; border-collapse: collapse; white-space:nowrap;}*/
       .sv-list td { line-height: 4px;}
       .sv-list td { border: 1px solid #777; border-collapse: collapse; white-space:nowrap;}
        .tdC_sel { background-color: #FFFF33; }
       .tablescroll_head {margin-left: 0px;}
       .SAVED {}
       .RUNNING {background-color:#FF3333;}
       ...
       .Params {}
        ...
        function listSavePoint() {
            $('#savepointDIV').html('');
            $.ajax({ url: contextRoot+'/savepoint/HandleSavePointAction.do?skipValidation=true&methodToCall=' + btnListSavePointL,
                    cache: false,
                    type: 'POST',
                    data: ({
                              id: $('#id').val(),
        ...
        function redrawList() {
            ...
            var node = $(templateList).bindTo(SavePoint, {root:'SavePoint',fill:'false',appendTo:'#savepointDIV'});
            $('#savepointDIV table tbody tr').each(function(){
                ...
                $(this).find('img').each(function(){
                    $(this).click(function() {showSavePointInfo(SavePoint.savepoint[$(this).parent().parent().attr('idx')]); });
                    $(this).mouseover(function() { $(this).css('cursor', 'hand'); });
                    $(this).mouseout(function() { $(this).css('cursor', 'pointer'); });
                });
            });
            $('#savepointDIV table.sv-list').tableScroll({height:300,containerClass:'sv-list'});
        }

        function recoverSavePoint() {
            $.ajax({ url: contextRoot+'/savepoint/HandleSavePointAction.do?skipValidation=true&methodToCall=' + btnRecoverSavePointL,
                    cache: false,
                    type: 'POST',
                    data: ({
                              rec_id: $('#rec_id').val()
        ...
        function deleteSavePoint() {
            $.ajax({ url: contextRoot+'/savepoint/HandleSavePointAction.do?skipValidation=true&methodToCall=' + btnDeleteSavePointL,
                    cache: false,
                    type: 'POST',
                    data: ({
                              rec_id: $('#rec_id').val()
        ...
        function showSavePointInfo(sp) {
            ...
            var buttons = {};
            buttons["Close"] = function() { $('#InfoPopup').dialog('close'); };
...
<table style="margin-left: 100px">
    <tr><td>
    <div id="filterDIV" style="margin-left: 100px; width: 600px">
