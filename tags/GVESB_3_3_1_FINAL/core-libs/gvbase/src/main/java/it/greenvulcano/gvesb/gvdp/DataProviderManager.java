...
{
    private static final Logger               logger                 = GVLogger.getLogger(DataProviderManager.class);
    ...
    public IDataProvider getDataProvider(String dpName) throws DataProviderException
    {
        ...
        if (iDataProvider == null) {
            ...
            String dpClass = "";
            try {
                dpClass = XMLConfig.get(dpConfigNode, "@class");
                Class<?> objEx = Class.forName(dpClass);
                iDataProvider = (IDataProvider) objEx.newInstance();
                iDataProvider.init(dpConfigNode);
                logger.debug("New data provider created: " + dpClass);
            }
            catch (Exception exc) {
                logger.error("Creation error: '" + exc.getMessage() + "' - DP class '" + dpClass
                ...
                throw new DataProviderException(DP_CREATION_ERROR,
                        new String[][]{
                                ...
                                {
                                        "cause",
                                        "Creation error: '" + exc.getMessage() + "' - IDataProvider class '" + dpClass
                                                + "'"}}, exc);
            }
        }
        else {
