 ...
 *         REVISION OK
 */
public class JMXSetOperation implements JMXMethodOperation
{
    /**
    ...
    {
        try {
            attributeName = XMLConfig.get(config, "@name");
            attributeValue = XMLConfig.get(config, "@value");
            attributeType = XMLConfig.get(config, "@value-type");
        }
        catch (Exception exc) {
    ...
    public Object perform(String objectName, Map<String, Object> props, GVBuffer gvBuffer) throws Exception
    {
        ...
        if (!PropertiesHandler.isExpanded(attributeValue)) {
            value = PropertiesHandler.expand(attributeValue, props, gvBuffer);
        }
        JMXUtils.set(objectName, attributeName, JMXCallUtils.cast(value, attributeType), logger);
        return null;
    }
