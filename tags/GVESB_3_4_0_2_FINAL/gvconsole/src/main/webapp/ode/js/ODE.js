...
(function(){
	...
	var undeployVar = 'undeployVar'+i;
        ...
        callDetail= 'org.apache.ode.ProcessHandling.viewProcessDetails("'+ process.nameWithVer +'","'+ process.urlOfName +'","'+process.prefixOfName+'")';
	callRetire= 'org.apache.ode.ProcessHandling.retireProcess("'+ process.nameWithVer +'","'+ process.urlOfName +'","'+process.prefixOfName+'")';
	callActivate= 'org.apache.ode.ProcessHandling.activateProcess("'+ process.nameWithVer +'","'+ process.urlOfName +'","'+process.prefixOfName+'")';
	callUndeploy= 'org.apache.ode.ProcessHandling.undeployPackage("'+ process.nameWithVer +'","'+ process.urlOfName +'","'+process.prefixOfName+'")';
	//callUndeploy= 'org.apache.ode.DeploymentHandling.undeployPackage("callOde_RequestReply_HelloWorld")';
        ...
        str = '<div class="yui-cms-item yui-panel selected"><div class="hd">'+
              ...
              '<span id="'+ retireBtnID +
              '" class="yui-button yui-push-button"><span class="first-child"><input type="button" name="'+ retireBtnID +'name" value="Retire" onclick='+callRetire+'></span></span>'+
              '<span id="'+ activateBtnID +
              '" class="yui-button yui-push-button"><span class="first-child"><input type="button" name="'+ activateVar +'name" value="Activate" onclick='+callActivate+'></span></span>'+
              '<span id="'+ viewProDetID +
              '" class="yui-button yui-push-button"><span class="first-child"><input type="button" name="'+ viewProDetVar +'name" value="Details" onclick='+callDetail+'></span></span>'+
	      '<span id="'+ undeployID +
              '" class="yui-button yui-push-button"><span class="first-child"><input type="button" name="'+ undeployVar +'name" value="Udeploy" onclick='+callUndeploy+'></span></span>'+
            ...
            function handleYes(){
                ...
                if (defInfoEle) {
                    ...
                    if (processNameEle) {
                        ...
                        if (substr(0,strlen(proNameFromRes),prefixWithName) == proNameFromRes) {
    ...
    function viewProcessDetails(processName, url, prefix){
	    ...
	    var processInfoEle = org.apache.ode.DOMHelper.getElementsByTagName(
			...
			var responseDefinition = ProcessManagementService.getProcessDefinition(processName, url, prefix);
                        var processDefStr = ProcessManagementService.text.replace(/>/g, '>\n');
    ...
    function undeployPackage(processName, url, prefix){
        try{
            function handleYes(){
                var response;
                try{
		  ProcessManagementService.undeploy.callback = null;
		  response = ProcessManagementService.undeploy(processName, url, prefix);
		} catch (e) {
                    if (typeof(e) == "string") {
                        org.apache.ode.Widgets.alert("Exception occured:\n" + e.toString());
                    }
                ...
                org.apache.ode.Widgets.alert('Package '+ processName + ' undeployed successfully.');
            }
            function handleNo(){
                org.apache.ode.Widgets.alert('Package undeployment cancelled!');
            }

            var msg = 'Dou you want to undeploy package ' + processName + '?';
...
(function(){
    ...
    function createInstanceWidget(instance, i){
        ...
        callTerminate= 'org.apache.ode.InstanceHandling.terminateInstance("'+ instance.iid +'")';
	callSuspend= 'org.apache.ode.InstanceHandling.suspendInstance("'+ instance.iid +'")';
	callResume= 'org.apache.ode.InstanceHandling.resumeInstance("'+ instance.iid +'")';
        str = '<div class="yui-cms-item yui-panel selected"><div class="hd">Instance ID: '+
              ...
              '" class="yui-button yui-push-button"><span class="first-child"><input type="button" name="'+ terminateBtnID +'name" value="Terminate" onclick='+callTerminate+'></span></span>'+
              '<span id="'+ suspendBtnID +
              '" class="yui-button yui-push-button"><span class="first-child"><input type="button" name="'+ suspendBtnID +'name" value="Suspend" onclick='+callSuspend+'></span></span>'+
              '<span id="'+ resumeBtnID +
              '" class="yui-button yui-push-button"><span class="first-child"><input type="button" name="'+ resumeBtnID +'name" value="Resume" onclick='+callResume+'></span></span>'+
              '</div> <div class="actions"><a href="#" class="accordionToggleItem">&nbsp;</a>'+
              '</div></div>'
