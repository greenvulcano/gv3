...
public class FileUIDCacheManager implements UIDCacheManager
{
    ...
    private final HashMap<String, Set<String>> popCacheMap              = new HashMap<String, Set<String>>();
    private String                       popCacheFolder           = System.getProperty(POP_CACHE_FOLDER_NAME,
                                                                          System.getProperty("java.io.tmpdir")
                                                                                  + File.separator + "GV_POP_CACHE");
    private final boolean                      popCacheFolderReadAlways = Boolean.getBoolean(POP_CACHE_READ_ALWAYS);
    ...
    {
        try {
            this.popCacheFolder = PropertiesHandler.expand(this.popCacheFolder);
            File popF = new File(this.popCacheFolder);
            if (!popF.isDirectory()) {
                if (!popF.mkdirs()) {
                    logger.error("Unable to create POP cache folder [" + this.popCacheFolder + "]");
                }
            }
        }
        catch (Exception exc) {
    ...
    private Set<String> getUIDCacheInt(String key) throws Exception
    {
        ...
        Set<String> uidCache = this.popCacheMap.get(key);

        if ((uidCache == null) || this.popCacheFolderReadAlways) {
            uidCache = null;
            String fileName = this.popCacheFolder + File.separator + key + ".uidcache";
            ...
            if (uidCacheFile.canRead()) {
                ...
                try {
                    ...
                    this.popCacheMap.put(key, uidCache);
                }
                catch (Exception exc) {
