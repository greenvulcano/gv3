    ...
    public void endElement(String uri, String localName, String qName) throws SAXException
    {
        if (ROW_NAME.equals(localName)) {
            ...
            if (this.currCriticalError) {
                ...
                this.resultMessage.append("Data error on row ").append(this.rowCounter).append(": ").append(msg).append('\n');
            ...
            if (sqlStatement_update != null) {
                ...
                try {
                    ...
                    if (isTransacted()) {
                        ...
                        throw new SAXException(new DBOException("SQLException error on update for row " + this.rowCounter
                                + ": " + exc.getMessage(), exc));
                    }
                    ...
                    if (isBlockingError(oraerr.getErrorType())) {
                        ...
                        logger.error("Record update parameters:\n" + dumpCurrentRowFields());
                        logger.error("SQL Statement Informations:\n" + this.sqlStatementInfoUpdate);
                        logger.error("SQLException configured as blocking error for service '" + this.serviceName
                                + "' on row " + Long.toString(this.rowCounter) + ".", exc);
                        throw new SAXException(new DBOException(
                                "SQLException configured as blocking error class on row " + this.rowCounter + ": "
                                        + exc.getMessage(), exc));
                    }

                    this.resultMessage.append("SQLException error on update for row ").append(this.rowCounter).append(": ").append(
                            exc.getMessage()).append('\n');
                    this.resultMessage.append("SQL Statement Informations:\n").append(this.sqlStatementInfoUpdate);
            ...
            if ((updated < 1) && (this.sqlStatementInfo != null)) {
                try {
                    int actualOk = 0;
                    this.localCurrentRowFields = this.currentInsertRowFields;
                    actualOk = ((PreparedStatement) this.sqlStatementInfo.getStatement()).executeUpdate();
